            ...
            if PeriodicitatPres.objects.filter(id_projecte=projecte.id_projecte):#Si hay periodos
                for periode in PeriodicitatPres.objects.filter(id_projecte=projecte.id_projecte):
                    ...
                    for partidaperio in PeriodicitatPartida.objects.filter(id_periodicitat=periode.id_periodicitat):#partida de x periodo
                        ...
                        gastat=0
                        cuenta=0
                        for compte in Desglossaments.objects.filter(id_partida=partidaperio.id_partida.id_partida):
                        ...
                        partidas.append({"descripcio":desc_partida,"pressupostat":pressupostat,"gastat":gastat,"saldo":saldo,"partidaperio":partidaperio.id_partida.id_partida,"cod":codigo_entero})
                    #redondeo totales
                    ...
                    #
                    periodos.append({"partides":partidas,"num_periode":n_periode,"total_pressupostat":total_pressupostat,"total_gastat":total_gastat,"total_saldo":total_saldo, "data_min":periode.data_inicial, "data_max":periode.data_final})
                ...
                for partida in Pressupost.objects.filter(id_projecte=projecte.id_projecte):#partidas de proyecto
                        ...
                        for compte in Desglossaments.objects.filter(id_partida=partida.id_partida):
                            ...
                            cursor.execute("SELECT DEBE,HABER,DESCAPU FROM __ASIENTOS WHERE CENTROCOSTE2='   '+(?) AND ( CONVERT(date,FECHA,121)<=(?) AND CONVERT(date,FECHA,121)>=(?) ) AND TIPAPU='N' AND IDCUENTA IN (SELECT IDCUENTA FROM CUENTAS WHERE CUENTA LIKE (?)+'%' ) ",[codigo_entero,fecha_max,fecha_min,cod_compte]) # AND ( FECHA<'2017-01-01 00:00:00.000' )
