...
class ProjectesForm(forms.ModelForm):  # hereda de la clase predefinida forms.ModelForm
    class Meta:
        ...
        widgets = {
            ...
            # 'id_resp': Select(),
            # 'id_usuari_extern': Select(choices=TUsuarisExterns.objects.all().order_by("nom_usuari_extern").values_list('id_usuari_extern','id_organisme__nom_organisme')),
            # 'id_usuari_extern': forms.ModelChoiceField(queryset=TUsuarisExterns.objects.all().order_by("nom_usuari_extern"),empty_label="--------"),
