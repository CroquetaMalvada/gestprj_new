...
@login_required(login_url='/menu/')
def AjaxListPciCabecera(request,id_grup,fecha_min_pci,fecha_max_pci): # AJAX PARA LOS PROYECTOS CON X ORGANIZACION FINANCIERA


    fecha_min = datetime.strptime(fecha_min_pci, "%d-%m-%Y")
    fecha_max = datetime.strptime(fecha_max_pci, "%d-%m-%Y")
    #pillamos los proyectos abiertos(se ve que en el gestor antiguo es lo que hace)
    #id_organisme=request.POST["id_organisme"]
...
def AjaxListResumFitxaMajorPrjDatos(request,fecha_min,fecha_max,codigo):
    ...
    for prjfet in projectfetch:
        ...
        # SELECT NOMPRO,LINEALBA.CTACONL,LINEALBA.TEXTO,LINEALBA.BASE,LINEALBA.CENTROCOSTE2,SITUACIONDETALLE FROM CABEALBC INNER JOIN LINEALBA ON CABEALBC.IDALBC=LINEALBA.IDALBC WHERE SITUACIONDETALLE!='SERVIDO, TOTALMENTE SERVIDO' AND SITUACIONDETALLE!='ANULADO, TOTALMENTE ANULADO' AND LINEALBA.CENTROCOSTE2='   03589'
    ...
    try:
        ...
        if tipo_comp == "2":  # ALBARANES
            ...
            try:
                for compte in comptes:
                    ...
                    cursor.execute(
                        "SELECT DISTINCT(NOMPRO),RAZON,CABEALBC.BASE,CABEALBC.TOTBRUTO,CABEALBC.IDALBC,SITUACIONDETALLE FROM CABEALBC INNER JOIN LINEALBA ON CABEALBC.IDALBC=LINEALBA.IDALBC WHERE SITUACIONDETALLE!='SERVIDO, TOTALMENTE SERVIDO' AND SITUACIONDETALLE!='ANULADO, TOTALMENTE ANULADO' AND LINEALBA.CENTROCOSTE2='   '+%s AND LINEALBA.CTACONL=%s",
        ...
        if tipo_comp == "3":#PEDIDOS
            ...
            try:
                for compte in comptes:
                    # obtenemos las cabeceras de los albaranes,para que se vean las lineas de los albaranes lo pondremos aparte
                    # "SELECT NOMPRO,RAZON,CABEPEDC.BASE,CABEPEDC.TOTBRUTO,CABEPEDC.IDPEDC,SITUACIONDETALLE FROM CABEPEDC INNER JOIN LINEPEDI ON CABEPEDC.IDPEDC=LINEPEDI.IDPEDC WHERE SITUACIONDETALLE!='SERVIDO, TOTALMENTE SERVIDO' AND SITUACIONDETALLE!='ANULADO, TOTALMENTE ANULADO' AND LINEPEDI.CENTROCOSTE2='   '+%s AND LINEPEDI.CTACONL=%s",
                    cursor.execute(
                        "SELECT DISTINCT(NOMPRO),RAZON,CABEPEDC.BASE,CABEPEDC.TOTBRUTO,CABEPEDC.IDPEDC,SITUACIONDETALLE FROM CABEPEDC INNER JOIN LINEPEDI ON CABEPEDC.IDPEDC=LINEPEDI.IDPEDC WHERE SITUACIONDETALLE!='SERVIDO, TOTALMENTE SERVIDO' AND SITUACIONDETALLE!='ANULADO, TOTALMENTE ANULADO' AND LINEPEDI.CENTROCOSTE2='   '+%s AND LINEPEDI.CTACONL=%s",
                        (codigo_entero, compte))
